...
# Sweep over tau values
for tau in tqdm(tau_values, desc="Tau sweep"):
    style_counts = {style: 0 for style in attachment_styles}
    total_agents = 0

    for _ in range(iters):
        # Run the simulation with specified tau
        subprocess.run([
            "./tmtSimulator",
            "-numAgents=40",
            "-iters=200",
    ...
    for style in attachment_styles:
        ...
        results.append((tau, style, avg))

# Create DataFrame
df = pd.DataFrame(results, columns=["tau", "AttachmentStyle", "Count"])

# Plot
plt.figure(figsize=(10, 6))
sns.lineplot(data=df, x="tau", y="Count", hue="AttachmentStyle", marker="o")
plt.xlabel(r"ASP Threshold $\tau$")
plt.ylabel("Avg Count of Agents (Final Iteration)")
plt.title("Attachment Style Survival vs ASP Threshold")
plt.grid(True)
plt.tight_layout()
plt.show()