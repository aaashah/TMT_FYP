...
app.layout = html.Div([
    ...
    html.Div([
        html.Button("⬅️ Previous", id="prev-turn", n_clicks=0, style={"font-size": "20px"}),
        html.Button("▶ Play", id="play-pause", n_clicks=0, style={"font-size": "20px", "margin": "0 10px"}),
        html.Button("Next ➡️", id="next-turn", n_clicks=0, style={"font-size": "20px"})
    ], style={"display": "flex", "justify-content": "center", "margin-bottom": "10px"}),

    html.Div(id="iteration-turn-label", style={"textAlign": "center", "font-size": "20px"}),
    ...
    dcc.Interval(id="animation-interval", interval=1000, n_intervals=0, disabled=False)
])
...
def update_grid(prev_clicks, next_clicks, n_intervals, current_iteration, current_turn):
    ...
    for t in tombstones:
        fig.add_annotation(x=t["X"], y=t["Y"], text="💀", showarrow=False, font=dict(size=15))

    for t in temples:
        fig.add_annotation(x=t["X"], y=t["Y"], text="🏛️", showarrow=False, font=dict(size=15))

    fig.update_layout(
        title=f"Iteration {current_iteration} - Turn {current_turn}",
        xaxis=dict(range=[-0.5, GRID_WIDTH + 1], tickvals=[i + 0.5 for i in range(GRID_WIDTH + 1)],
                ticktext=[str(i) for i in range(GRID_WIDTH + 1)], showgrid=True, gridcolor="lightgray"),
        yaxis=dict(range=[-0.5, GRID_HEIGHT + 1], tickvals=[i + 0.5 for i in range(GRID_HEIGHT + 1)],
                ticktext=[str(i) for i in range(GRID_HEIGHT + 1)], showgrid=True, gridcolor="lightgray"),
        height=GRID_HEIGHT * CELL_SIZE, width=GRID_WIDTH * CELL_SIZE,
        showlegend=True, plot_bgcolor="white", autosize=True
    )

    return fig, f"Iteration {current_iteration} - Turn {current_turn}", current_iteration, current_turn
